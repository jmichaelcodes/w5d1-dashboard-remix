...
changeSource(0, 'winter', 'Winter');
changeSource(1, 'spring', 'Spring');
changeSource(2, 'summer', 'Summer');
changeSource(3, 'fall', 'Fall');

function changeSource(index, season, alt) {
    images[index].src = `https://robohash.org/${season}`;
...
function rowIDs(position, headerName) {
    return headingRow.children[position].innerText = headerName;
}
...
for (var i = 1; i < rowArray.length; i++) {
    ...
    var object = {};
    object.id = Number(rowArray[i].children[0].innerText.replace(/,/g, ''));
...
function allItems(position) {
    ...
    items.classList = "text-capitalize";
}
